...
#pragma omp single
		{
			messageStream << "Begin clustering ";
			numThreads = omp_get_num_threads();
			result.numThreads = numThreads;
			if (numThreads == 1)
				messageStream << "using 1 thread.\n";
			else
				messageStream << "using " << numThreads << " threads.\n";
			messageStream << "Number of permutations to search: "
			...
			*/
			...
			for (ind_t k = 1; k < N; ++k)
			{
				...
				for (ind_t t = 0; t < tryNumClusts; ++t)
				{
					...
					if (tmpLabelDelta > bestLabelDelta)
					{ // want to maximise the change
...
#pragma omp simd
	...
	/* OUTPUT IF NOT IN R */
#ifndef HAS_RCPP
	messageStream << "Cluster labels:\n"
				  << result.labels;
	messageStream << "Finished clustering, found " << result.numClusts
				  << " clusters.\n";
	messageStream << "Normalised binder loss: " << result.binderLoss << '\n';
	messageStream << "Number of permutations scanned: " << result.nIters
				  << '\n';
	messageStream << "Time limit reached: " << result.timeLimitReached;
#endif
	return result;
}
